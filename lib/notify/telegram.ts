...
export async function forwardIncoming(update: unknown): Promise<void> {
  ...
  // ЧЕЛОВЕКУ НАДО ОТВЕТИТЬ, А НЕ ТОЛЬКО ПЕРЕСЛАТЬ ЕГО СЛОВА НАМ.
  //
  // До этого сообщение уходило в группу, а отправитель не видел ничего —
  // ни галочки, ни слова. Со стороны это неотличимо от «бот сломан»: человек
  // написал в поддержку продукта, который продаёт внимание к людям, и получил
  // тишину. Пересылка без ответа хуже, чем отсутствие бота: она создаёт
  // ощущение, что его прочитали и проигнорировали.
  await acknowledge(token, message.chat?.id);
}

/**
 * Подтверждает получение — не чаще раза в час на собеседника.
 *
 * Окно нужно, потому что люди пишут очередями: три сообщения подряд, уточнение
 * следом. Отвечать на каждое одной и той же строкой — то же самое, что не
 * отвечать: текст перестают читать. Одно подтверждение на разговор, а дальше
 * человек ждёт живого ответа, который и обещан.
 *
 * Счётчик — тот же, что у ограничения частоты: при живом Upstash окно общее
 * для всех инстансов, без него — своё у каждого. Хуже всего, что тут может
 * случиться, — второе подтверждение из другого инстанса; ради этого заводить
 * отдельное хранилище не стоит.
 */
async function acknowledge(token: string, chatId: number | undefined): Promise<void> {
  if (!chatId) return;

  const attempt = await rateLimit(SUPPORT_ACK, String(chatId));
  if (!attempt.ok) return;

  await reply(token, chatId, ackText());
}

/** Правило «одно подтверждение в час на собеседника». */
const SUPPORT_ACK = { name: 'support-ack', limit: 1, windowMs: 60 * 60_000 } as const;

/**
 * Что именно отвечаем.
 *
 * Без обещаний, которых некому держать: «ответим в течение дня» — это то, что
 * уже написано в приветствии бота, и держать это должен живой человек. Если
 * окажется, что не держим, строку надо менять здесь, а не оправдываться перед
 * людьми поодиночке.
 */
function ackText(): string {
  return [
    'Получили — прочитаем и ответим здесь же, в течение дня.',
    '',
    'Если вопрос про заказ, пришлите код с бирки: шесть или восемь знаков,',
    'например <code>A1B2C3D4</code>. С ним найдём быстрее.',
  ].join('\n');
}
