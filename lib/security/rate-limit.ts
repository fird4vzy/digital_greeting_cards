...
export const RATE_LIMITS = {
  ...
  /**
   * Выдача токена на загрузку фотографии.
   *
   * Щедрее остальных намеренно: заказ несёт до двенадцати снимков, и каждый
   * просит свой токен, так что живой заказчик за один заход берёт дюжину.
   * Сорок за час — это три полных заказа подряд с одного адреса.
   *
   * Ограничение здесь единственное, что стоит между открытой формой и чужим
   * файлохранилищем: маршрут не требует входа, потому что заказ оформляет
   * человек с улицы.
   */
  photoUpload: { name: 'photo', limit: 40, windowMs: 60 * 60_000 },
