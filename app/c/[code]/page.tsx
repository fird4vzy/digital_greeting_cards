...
export default async function CardPage({ params }: Props) {
   ...
   *
   * Реферер читается **до** `after`, а не внутри: Next запрещает трогать
   * `headers()` в отложенном обратном вызове и роняет его целиком — проверено
   * на боевых логах, первые три сканирования не записались именно так.
   * Внутрь уезжает уже готовое значение.
   *
   * Сам реферер — только чужой. Свой домен в статистике не значит ничего, а
   * пишется он затем, чтобы знать, откуда переходят: из камеры, из телеграма,
   * из переписки.
   */
  const referrer = (await headers()).get('referer');
  const external = referrer && !referrer.includes('/c/') ? referrer.slice(0, 500) : null;

  after(() => recordCardView(order.code, external));
