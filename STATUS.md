...
**Last updated:** 28 August 2026 — **связь с ботом теперь двусторонняя**:
заказы уходят в супергруппу, а написанное боту приходит туда же. Осталось
зарегистрировать адрес вебхука — три команды, они ниже. Оба вопроса, которые держали работу, закрыты
...
### Шаг 3. Включить вебхук — три команды

Решено 28 августа: бот не заменяется человеком, а **учится принимать**.
...
1. **Придумать секрет и положить его в Vercel** как `TELEGRAM_WEBHOOK_SECRET`
   ...
   Без секрета маршрут `/api/telegram/webhook` отвечает 404 и ничего не
   принимает — это не забывчивость, а защита: открытый вебхук позволил бы
   любому прохожему слать в рабочую группу что угодно от имени бота.
2. **Зарегистрировать адрес у Telegram:**
   `npm run tg:webhook set https://digital-greeting-cards-ochre.vercel.app`
3. **Проверить:** напишите боту в личку — сообщение должно прийти в группу с
   шапкой «Написали боту», именем и username отправителя.

`npm run tg:webhook status` покажет, что зарегистрировано и была ли ошибка
доставки; `delete` снимет.

**Одна несовместимость, о которой надо знать заранее.** Пока вебхук стоит,
кнопка «Найти chat id» в `/admin` будет отвечать 409: сообщения забирает
вебхук, и опросить их вторым способом Telegram не даёт. Это не поломка. Нужен
chat id — `npm run tg:webhook delete`, найти, `set` обратно.

Отдельно осталось решить **цену**: в `lib/shops/offer.ts` всё ещё 25% и 15
бесплатных открыток, помеченные как предварительные. Это решение, а не задача,
но принять его надо до показа магазинам.
