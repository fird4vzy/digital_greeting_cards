...
### Шаг 1. Доделать ротацию доступов

Пароль админки **сменён** 28 августа. Осталось из той же истории — исходить
надо из того, что утечь могло всё, потому что `GET /api/orders` был открыт
столько, сколько существовал, и логов, по которым видно скачивание, нет:

1. **Перевыпустить токен бота** у @BotFather (`/revoke` → `/token`), положить
   новый в `TELEGRAM_BOT_TOKEN` в Vercel, Redeploy, затем
   `npm run tg:webhook set https://<домен>` — старый вебхук отвалится вместе
   со старым токеном.
2. **Задать `ADMIN_SESSION_SECRET`** (`node -e "console.log(crypto.randomUUID())"`).
   Пока его нет, ключом подписи сессий служит сам пароль, и каждая выданная
   кука годится для его офлайнового подбора.
