...
2. **Починить уведомления. Причина уже найдена, осталось три действия.**
   ...
   (1) ткнуть в участника «delovoybot» в группе и сверить username — должен
   быть `@birdunyobot`; чужого убрать, нужного добавить;
   (2) открыть группу в web.telegram.org, взять из адресной строки `-100…` —
   это и есть правильный `TELEGRAM_CHAT_ID` (у группы включены топики, значит
   она супергруппа, а при превращении chat id меняется, и старый мёртв);
   (3) обновить переменную в Vercel для Production → **Redeploy** → кнопка
   проверки в `/admin`, она покажет отказ Telegram словами.
...
### Шаг 3. Решить, куда пишет флорист, перед показом магазинам

`lib/shops/offer.ts`, поле `telegram`. Прежний `@birdunyo` был выдуман и
открывался ошибкой; сейчас там `@birdunyobot` — адрес существует. Но **бот
только отправляет**: у него нет ни опроса, ни вебхука, поэтому написанное ему
не увидит никто. Флорист напишет и не получит ответа — это хуже неработающей
ссылки, потому что выглядит рабочим.

Два честных выхода: поставить туда личный username живого человека — одна
строка, работает сегодня; или сделать вебхук, который пересылает сообщения
боту в ту же группу (маршрут, регистрация адреса у Telegram, проверка
секретного токена в заголовке). До этого `/shops` магазинам не раздавать.
Там же лежат все цифры предложения, помеченные как предварительные.
...
## «Собрать открытку» ничего не ломает — и не должна ничего открывать

Возвращается третий раз, поэтому записано отдельно.

**Кнопка работает.** Она пересобирает открытку из ответов и пишет результат в
базу. Она **не открывает** ничего и не должна: открывает соседняя —
«Посмотреть черновик».
