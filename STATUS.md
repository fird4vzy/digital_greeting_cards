...
1. **Нет ограничения частоты нигде.** Пароль админки один на весь бизнес, и
   его можно перебирать сколько угодно: ни задержки, ни блокировки, ни записи
   в лог. То же на создании заказов — бесконечная запись в базу и бесконечные
   сообщения в рабочую группу.
2. **Тихое падение в память.** Если `DATABASE_URL` не задан или `pg` не
   загрузился, приложение молча переходит на файловое хранилище, на Vercel
   read-only, то есть в память одного инстанса. Заказчик видит «готово»,
   заказ уходит в никуда, в телеграм приходит ссылка на 404. Один
   `console.warn` на холодном старте — весь сигнал. Надо падать громко.
3. **`/u/` отдаёт чужой HTML с нашего домена.** Во фрейме песочница честная,
   но прямой переход по ссылке песочницы не имеет, и скрипт выполняется на
   birdunyo.uz с кукой оператора. Нужен отдельный поддомен или CSP `sandbox`.
4. **Нет ни одного теста.** Первым — матрица прав: каждый маршрут × {без куки,
   поддельная кука, живая кука}. Она одна поймала бы всё, что чинили сегодня.
5. **Ни мониторинга, ни `/api/health`, ни трекинга ошибок.** Пункт 2 без этого
   не заметить.
6. Фотографии лежат base64 в JSONB: страница очереди читает 50 заказов со
   всеми снимками. Нужен объектный сторадж.
7. `React.cache()` на чтение открытки — сейчас три запроса и два полных скана
   шаблонов на один скан QR.
8. Приватность: нет политики хранения, нет удаления по запросу, нет согласия
   получателя. Это работа не по коду.
