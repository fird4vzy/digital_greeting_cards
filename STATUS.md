...
### Шаг 3a. Применить миграцию 007 — иначе телеграм заказчика не сохранится

`npm run db:migrate`. Добавляет `orders.customer_telegram`. Код к её
отсутствию терпим — поле просто не пишется, — но и контакта тогда не будет,
а это единственный способ связаться с заказчиком.
