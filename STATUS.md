...
### Что этим ещё не починено

Своя открытка (папка руками) грузится прежним путём, через
`/api/admin/card-files`, и упирается в те же 4,5 МБ — из семи работ три не
проходят. Переезд туда же напрашивается, но трогает отдачу `/u/<код>` и её
песочницу, то есть безопасность; делать это заодно, в одном заходе с
фотографиями, было бы неаккуратно.
