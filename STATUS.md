...
2. **Починить уведомления. Причина уже найдена, осталось три действия.**
   ...
   (2) **нажать «Найти chat id» в `/admin`** — кнопка спрашивает у Telegram,
   какие чаты бот видел, и показывает их id, пометив тот, что настроен сейчас.
   Ручная охота в `web.telegram.org` больше не нужна. Если список пуст,
   напишите что-нибудь в группу и нажмите снова: Telegram помнит только сутки.
   Причина, по которой шаг вообще существует: у группы включены топики, значит
