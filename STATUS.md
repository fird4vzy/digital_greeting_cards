...
### Шаг 0. Сменить пароль админки и токен бота — сегодня

28 августа сторонний аудит нашёл, что `GET /api/orders` не проверял ничего:
один запрос без куки возвращал все заказы целиком — имена, телефоны,
телеграмы, письма получателям и фотографии. Маршрут был открыт столько,
сколько существовал. Дыры закрыты и проверены на бою (`/api/orders` отвечает
401), но **закрыть дыру — не то же самое, что отменить утечку.** Кто успел
скачать, тот скачал, и логов, по которым это видно, нет.

Поэтому исходить надо из того, что пароль админки и токен бота могли утечь:

1. **Сменить `ADMIN_PASSWORD`** в Vercel → Production, Redeploy. Смена пароля
   заодно разлогинивает все выданные сессии — это здесь и нужно.
2. **Перевыпустить токен бота** у @BotFather (`/revoke` → `/token`), положить
   новый в `TELEGRAM_BOT_TOKEN`, Redeploy, затем `npm run tg:webhook set
   https://<домен>` — старый вебхук отвалится вместе со старым токеном.
3. Заодно задать **`ADMIN_SESSION_SECRET`** (`node -e
   "console.log(crypto.randomUUID())"`). Пока его нет, ключом подписи сессий
   служит сам пароль, и каждая выданная кука годится для его офлайнового
   подбора.

Что уже сделано в коде — см. «Аудит 28 августа» ниже.
...
### ~~Шаг 3a. Миграция 007~~ — применена 28 августа

`orders.customer_telegram` на месте, проверено запросом. Телеграм заказчика
сохраняется и показывается оператору ссылкой.

**Заодно исправлена потеря, которая была до этого.** `wish` — «своя идея»
заказчика из шага 6 — читался и выбирался, но **никогда не вставлялся при
создании заказа**: до оператора он не доезжал. Причина в том, как была
написана вставка: имена колонок и значения жили двумя независимыми списками,
а номера плейсхолдеров считались вручную. Теперь колонка и значение
объявляются парой, и забыть значение нельзя, не убрав колонку.
...
Полный текст лежит вне репозитория: `Desktop/bir-dunyo-audit.md`. Оценка была
42/100 и вердикт «не готово», и основания были.

**Закрыто и проверено на продакшене:**

| Что | Было |
|---|---|
| `GET /api/orders` | Отдавал все заказы кому угодно. `?limit=` пустым снимал лимит совсем: `Number('')` — ноль, ноль ложен, ложный лимит значил «без LIMIT» |
| `GET`/`PATCH /api/orders/[id]` | Маршрут удалён целиком. Позволял анониму опубликовать чужой черновик, снять с публикации живую открытку и переписать текст |
| Сессия админки | Без `ADMIN_PASSWORD` в проде кука, подписанная строкой из этого же репозитория, открывала всё. Комментарий рядом обещал обратное |
| `publish` в теле запроса | `curl -d '{"publish":true}'` публиковал мимо просмотра магазином. Поля больше нет |
| Коды открыток | `Math.random()` предсказуем, а `generateId` раздавал его состояние наружу. Теперь `randomInt`, новые коды восьмизначные |
| Повтор при коллизии кода | Пять раз слал в базу тот же занятый код |
| `/api/ai/plan` | Открытый вызов Opus с `max_tokens: 8000` без лимитов |
| QR отменённого заказа | Показывал имена получателя и заказчика после отмены |
| `remotePatterns: '**'` | `/_next/image` работал открытым прокси-ресайзером для всего интернета |
...
**Закрыто во второй заход, 28 августа, проверено на бою:**

| Что | Как теперь |
|---|---|
| Ограничения частоты не было нигде | Вход — 5 попыток за 15 минут, создание заказов — 10 в час, планировщик — 30 в час. Счёт общий, если заданы `UPSTASH_REDIS_REST_*`, иначе в памяти инстанса |
| Тихое падение в память | В проде нет базы — нет обслуживания. Плюс `SELECT 1` при подключении: `new Pool()` с неверной строкой не падает сам |
| `/u/` без песочницы при прямом переходе | Заголовок CSP `sandbox allow-scripts allow-popups` на самом ответе — действует и во фрейме, и по прямой ссылке |
| Ни одного теста | 37 тестов, ноль новых зависимостей (`npm run check`). Матрица прав по маршрутам, сессия админки, правила заказа |
| Нечем следить | `/api/health` с настоящим запросом в базу; отвечает 503, если пути до неё нет |
| Три запроса и два скана таблицы на один скан QR | `React.cache()` на чтение заказа и на список шаблонов оператора |
| Кеш «колонки нет» навсегда | Кешируется только «есть». Раньше один разрыв связи на первой проверке убеждал инстанс, что колонки нет, — и заказы теряли поля без ошибки в логе |
| Двойное нажатие создавало два заказа | Ключ повторной отправки, миграция 008 **применена**. Заодно открытка собирается до вставки: окна, где заказ существует с пустой открыткой, больше нет |
| `card_views` пустовала с самого начала | Сканирования считаются, число видно в админке. Пишется код и время, ничего о человеке |
| Очередь читала всю таблицу заказов | По 25 на страницу. Лимита там не было вообще, а каждая строка несёт фотографии в base64 |
...
2. **Фотографии лежат base64 в JSONB.** Страница очереди читает 50 заказов со
   всеми снимками разом. Пока заказов двадцать — незаметно; на тысяче страница
   встанет. Нужен объектный сторадж, это дни работы, а не часы.
3. **Нет трекинга ошибок.** `/api/health` показывает, что база жива, но не
   покажет, что у каждого десятого заказа падает сборка открытки. Sentry
   требует аккаунт и DSN — ваша часть, дальше мой код.
4. **Приватность.** Нет политики хранения, нет удаления по запросу, нет
   согласия получателя — а его имя и фотографии обрабатываются, и он о нас не
   просил. Работа не по коду, но она нужна.
5. **Один пароль на все магазины.** Пока магазин один — это честное
