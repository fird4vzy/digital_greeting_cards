...
if (command === 'face') {
  /**
   * Как бот выглядит для того, кто открыл его впервые.
   *
   * Три разных текста, и это не дублирование — Telegram показывает их в
   * разных местах, и человек видит их по очереди:
   *
   *  - `short` — под именем в поиске и в списке чатов. Одна строка, её читают
   *    до того, как решат открывать.
   *  - `description` — на пустом экране до первого сообщения. Единственный
   *    момент, когда человек готов прочитать несколько строк.
   *  - `/start` — ответ на первое действие; отправляется вебхуком.
   *
   * Текст говорит и клиенту, и цветочному магазину, потому что приходят оба и
   * заранее не различить кто.
   */
  const site = origin || 'https://digital-greeting-cards-ochre.vercel.app';

  const short = 'Цифровые открытки к букетам. Напишите — ответим.';

  const description = [
    'Мы делаем цифровую открытку, которая едет вместе с букетом:',
    'маленькая бирка с кодом на стеблях, человек наводит телефон —',
    'и открывается то, что вы для него собрали.',
    '',
    'Если вы дарите — соберите открытку сами: ' + site + '/create',
    'Если у вас цветочный магазин — условия здесь: ' + site + '/shops',
    '',
    'Напишите сюда любой вопрос, мы отвечаем в тот же день.',
  ].join('\n');

  const start = [
    'Здравствуйте! Это Bir dunyo — цифровые открытки к букетам.',
    '',
  ...
  const results = await Promise.all([
    call('setMyShortDescription', { short_description: short }),
    call('setMyDescription', { description }),
    call('setMyCommands', {
      commands: [{ command: 'start', description: 'Что это и с чего начать' }],
    }),
  ]);

  const names = ['короткое описание', 'описание', 'команды'];
  results.forEach((out, i) => {
    console.log(out.ok ? `${names[i]}: обновлено` : `${names[i]}: отказ — ${out.description}`);
  });

  console.log('\nОтвет на /start живёт в коде (lib/notify/telegram.ts) и уходит вебхуком.');
  console.log('Проверьте: откройте бота в новом аккаунте или нажмите «Начать».');
  process.exit(results.every((r) => r.ok) ? 0 : 1);
}

console.error('Команды: status | set <https://домен> | delete | topics | face');
process.exit(1);
