...
const PUBLIC: Record<string, string> = {
  ...
  'app/api/photos/upload/route.ts':
    'Токен на загрузку снимка для формы заказа: её заполняет человек с улицы, ' +
    'входа у него нет. Байты сюда не приходят. Границы вместо сессии — только ' +
    'изображения трёх форматов, потолок 5 МБ и ограничение частоты по адресу; ' +
    'без BLOB_READ_WRITE_TOKEN маршрут отвечает 501 и не выдаёт ничего.',
};
